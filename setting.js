var PAR = {
					...
					success: function(d) {
						if (d.code == 23) {
							...
							$(a).each(function(index, item) {
								l = `<d class="danmuku-list" time="${item[0]}"><li>${PAR.formatTime(item[0])}</li><li title="${item[4]}">${item[4]}</li><li title="用户：${item[3]}  IP地址：${item[5]}">${item[6]}</li><li class="report" onclick="PAR.danmu.report(\'${item[5]}\',\'${b}\',\'${item[4]}\',\'${item[3]}\')">举报</li></d>`
								$(".list-show").append(l);
							})
						}
    ...
    'MYad': {
        'vod': function(o, b) {
            $("#ADtip").html('<a id="link" href="' + b + '" target="_blank">查看详情</a>');
